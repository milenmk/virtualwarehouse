...
/**
 * \file    virtualwarehouse/admin/setup.php
 * \ingroup virtualwarehouse
 * \brief   VirtualWarehouse setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "virtualwarehouse@virtualwarehouse"));
...
$arrayofparameters = [
    'SalesRepUserGroup' => ['css' => 'minwidth500', 'enabled' => 1],
];
...
if ($action == 'updateMask') {
    ...
    foreach ($dirmodels as $reldir) {
        $file = dol_buildpath($reldir . "core/modules/virtualwarehouse/doc/pdf_" . $modele . "_" . strtolower($tmpobjectkey) . ".modules.php", 0);
        if (file_exists($file)) {
            $filefound = 1;
    ...
    if (!empty($tmpobjectkey)) {
        $constforval = 'VIRTUALWAREHOUSE_' . strtoupper($tmpobjectkey) . "_ADDON";
...
elseif ($action == 'set') {
    ...
    if ($ret > 0) {
        $tmpobjectkey = GETPOST('object');
        if (!empty($tmpobjectkey)) {
            $constforval = 'VIRTUALWAREHOUSE_' . strtoupper($tmpobjectkey) . '_ADDON_PDF';
...
$page_name = "VirtualWarehouseSetup";
...
print dol_get_fiche_head($head, 'settings', '', -1, "virtualwarehouse@virtualwarehouse");

// Setup page goes here
echo '<span class="opacitymedium">' . $langs->trans("VirtualWarehouseSetupPage") . '</span><br><br>';
                    ...
                    while (($file = readdir($handle)) !== false) {
                        if (strpos($file, 'mod_' . strtolower($myTmpObjectKey) . '_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
                            ...
                            if ($module->isEnabled()) {
                                ...
                                $constforvar = 'VIRTUALWAREHOUSE_' . strtoupper($myTmpObjectKey) . '_ADDON';
